        ...
        {PLANNER_BLOCKS.map((b, i) => (
          <motion.div
            ...
            className={`flex items-start gap-2 px-2.5 py-2 rounded-xl border ${
              b.type === 'focus'   ? 'border-linen bg-linen' :
              b.type === 'meeting' ? 'border-linen bg-linen' :
              b.type === 'high'    ? 'border-linen bg-linen' :
              b.type === 'rest'    ? 'border-linen bg-transparent opacity-60' :
              'border-linen bg-linen'
            }`}
          >
            <span className="text-[9px] text-ink font-mono w-8 flex-shrink-0 mt-0.5">{b.time}</span>
            {b.type === 'focus' && (
              <div className={`w-0.5 self-stretch rounded-full flex-shrink-0 ${
                b.type === 'focus' ? 'bg-accent' : 'bg-accent'
              }`} style={{ minHeight: 14 }} />
            )}
            ...
            {b.tag && (
              <span className={`text-[9px] px-1.5 py-0.5 rounded-full font-medium flex-shrink-0 ${
                b.tag === 'HIGH' ? 'bg-linen text-accent' :
                b.tag === 'MTG'  ? 'bg-linen text-accent' :
                'bg-linen text-ink'
              }`}>{b.tag}</span>
            )}
          </motion.div>
          ...
          >
            <div className="flex items-baseline gap-1.5">
              <span className={`font-mono text-[10px] font-medium ${
                step.call.startsWith('notion') ? 'text-accent' :
                step.call.startsWith('cloud')  ? 'text-accent' :
                'text-ink'
              }`}>{step.call}</span>
              <span className="font-mono text-[9px] text-ink truncate max-w-[130px]">({step.args})</span>
            </div>
      ...
      <AnimatePresence>
        {showGaps && (
          <motion.div
            ...
            className="mt-2.5 pt-2.5 border-t border-linen"
          >
            {/* Summary */}
            <div className="flex items-center gap-2 px-1.5 py-1.5 rounded-lg bg-linen border border-linen mb-2">
            ...
            {GAP_TASKS.slice(0, 3).map((task, i) => (
              <motion.div
                ...
                className="flex items-center gap-2 px-2.5 py-1.5 rounded-lg border border-linen bg-linen mb-1"
              >
      ...
      <AnimatePresence>
        {demo.id === 'cloud-compare' && phase >= 0 && (
          <motion.div
            ...
            className="border-b border-linen px-4 py-2 bg-linen overflow-hidden"
          >
